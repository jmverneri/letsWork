...
<main class="page-root">
  ...
  <div class="page-header">
    <div>
      <h1 class="page-title"><?= htmlspecialchars($this->getCompany()->getName()) ?></h1>
  ...
  <div class="card" style="max-width:700px;">
    ...
    <div style="display:grid; grid-template-columns:1fr 2fr; gap:0.75rem 1rem; margin-bottom:1.25rem;">
      ...
      <span style="font-size:13px; font-family:monospace;"><?= htmlspecialchars($this->getCompany()->getCuit()) ?></span>

      <span class="app-label" style="align-self:center;">Ciudad</span>
      <span style="font-size:13px;"><?= htmlspecialchars($this->getCompany()->getCity() ?? 'N/A') ?></span>

      <span class="app-label" style="align-self:center;">Teléfono</span>
      <span style="font-size:13px;"><?= htmlspecialchars($this->getCompany()->getPhoneNumber() ?? 'N/A') ?></span>

      <span class="app-label" style="align-self:center;">Estado</span>
      <span><?= $this->getCompany()->isActive() ? '<span class="badge-active">Activa</span>' : '<span class="badge-inactive">Inactiva</span>' ?></span>
    ...
    <p style="font-size:13px; color:#37352f; line-height:1.7; background:#f7f6f3; border:0.5px solid #e0ddd8; border-radius:8px; padding:12px 14px; margin:0;">
      <?= nl2br(htmlspecialchars($this->getCompany()->getDescription() ?? 'Sin descripción disponible.')) ?>
    </p>

    <div style="display:flex; justify-content:flex-end; margin-top:1.5rem;">
      <a href="<?= FRONT_ROOT ?>StudentJobOffer/showOffersByCompany/<?= $this->getCompany()-> getCompanyId(); ?>" class="btn-dark-primary">
